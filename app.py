...
TASK_IDENTIFY_MOVEMENT = "Identify Potential Movement" # New Task
TASK_OPTIONS = [TASK_PARAGRAPH_SUMMARY, TASK_EXPLAIN_MAIN_SUBJECT, TASK_THIELIAN_LENS, TASK_IDENTIFY_MOVEMENT]
...
def get_llm_response(source_text, prompt_template_str, llm_choice_name, step1_output_for_step2=None):
    ...
    if any(keyword in prompt_template_str.lower() for keyword in ["explain", "explainer", "insight", "movement"]):
        api_max_tokens = 2048 
...
def run_app():
    ...
    with st.sidebar:
        ...
        current_llm_b_selection = st.selectbox(
            ...
            key="ui_llm_b_selector", help="Choose LLM for Step 2."
        )
    ...
    if st.session_state.get('run_processing_flag', False) and uploaded_files:
        ...
        if not api_keys_ok:
            ...
            with st.spinner(f"Processing {total_files} PDF(s) for '{task_for_this_run}' task..."):
                for i, uploaded_file in enumerate(uploaded_files):
                    current_file_result = {
                        "filename": uploaded_file.name, "task_performed": task_for_this_run,
                    ...
                    try:
                        ...
                        if extraction_error: current_file_result["error_message"] = extraction_error; st.session_state.results.append(current_file_result); continue 
                        ...
                        elif task_for_this_run == TASK_IDENTIFY_MOVEMENT: # New Task
                        ...
                        status_text_placeholder.text(f"Step 1 ({task_for_this_run}) for: {uploaded_file.name}...")
                        ...
                        if step1_error: current_file_result["error_message"] = f"Step 1 error: {step1_error}"; st.session_state.results.append(current_file_result); continue
                        current_file_result["step1_output"] = step1_output

                        status_text_placeholder.text(f"Step 2 (Verification) for: {uploaded_file.name}...")
                        ...
                        if verification_error: current_file_result["error_message"] = f"Step 2 error: {verification_error}"; current_file_result["step2_verification"] = "Verification failed."; st.session_state.results.append(current_file_result); continue
                        current_file_result["step2_verification"] = verification_output
                    
                    except Exception as e: current_file_result["error_message"] = f"Unexpected error: {str(e)}"
    ...
    if st.session_state.results:
        st.markdown("---"); st.header("📊 Processing Results")
        downloadable_content = "" 
        for i, res in enumerate(st.session_state.results): 
            task_label = res.get('task_performed', "N/A") 
            llm_a_used = res.get('llm_a_used', "N/A"); llm_b_used = res.get('llm_b_used', "N/A")
            expander_title = f"Results: {res['filename']} (Task: {task_label})"
            if res["error_message"]: expander_title += " (Error)"
            
            with st.expander(expander_title, expanded=False):
                downloadable_content += f"--- Results for: {res['filename']} ---\nTask: {task_label}\n\n"
                if res["error_message"]: st.error(f"Error: {res['error_message']}"); downloadable_content += f"Error: {res['error_message']}\n\n"
                ...
                if res["step1_output"]:
                    ...
                    if res["time_step1_sec"] is not None: st.caption(f"Time: {res['time_step1_sec']:.2f}s"); downloadable_content += f"Time: {res['time_step1_sec']:.2f}s\n"
                    st.markdown(res["step1_output"]); downloadable_content += f"{header_text}:\n{res['step1_output']}\n\n"
                elif not res["error_message"]: st.info("Step 1 output N/A."); downloadable_content += "Step 1 output N/A.\n\n"

                if res["step2_verification"]:
                    st.subheader(f"Step 2 Verification by {llm_b_used}:")
                    if res["time_step2_sec"] is not None: st.caption(f"Time: {res['time_step2_sec']:.2f}s"); downloadable_content += f"Time: {res['time_step2_sec']:.2f}s\n"
                    downloadable_content += f"Verification by {llm_b_used}:\n"
                    if "GREEN LIGHT" in res["step2_verification"].upper(): st.success("✅ GREEN LIGHT"); downloadable_content += "GREEN LIGHT\n"
                    elif "RED LIGHT" in res["step2_verification"].upper(): 
                        st.error("❌ RED LIGHT"); downloadable_content += "RED LIGHT\n"
                        try:
                            reasons = res["step2_verification"].upper().split("RED LIGHT", 1)[1].strip()
                            if reasons: st.markdown("**Reasons:**"); downloadable_content += "Reasons:\n"
                            for line in reasons.split('\n'):
                                clean_line = line.strip()
                                if clean_line.startswith("*") or clean_line.startswith("-"): st.markdown(clean_line); downloadable_content += f"{clean_line}\n"
                                elif clean_line: st.markdown(f"* {clean_line}"); downloadable_content += f"* {clean_line}\n"
                        except IndexError: pass 
                    else: st.info(res["step2_verification"]); downloadable_content += f"{res['step2_verification']}\n"
                    downloadable_content += "\n"
                elif not res["error_message"] and res["step1_output"]: st.info("Step 2 N/A."); downloadable_content += "Step 2 N/A.\n\n"
                downloadable_content += "\n" 

        if downloadable_content: 
            st_copy_to_clipboard(downloadable_content, "Copy All Results to Clipboard")
            st.caption("Click button to copy all results.")

    st.markdown("---")
    st.caption("Ensure API keys (GROQ_API_KEY, OPENAI_API_KEY) and PASSWORD are set as environment variables.")
    st.caption(f"Models: {', '.join(MODEL_OPTIONS.keys())} (Verify exact API IDs).")
    st.caption("`streamlit-copy-to-clipboard` needed: `pip install streamlit-copy-to-clipboard`")
